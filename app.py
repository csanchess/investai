...
import gpt_2_simple as gpt2   # same import, works with gpt-2-simple-tf2
import os
from build_dataset import build_financial_esg_corpus

st.title("💹 ESG & Financial AI Assistant (GPT-2, TF2-compatible)")
...
if st.sidebar.button("📘 Build ESG-Financial Dataset"):
    ...
    dataset_path = build_financial_esg_corpus(
        tickers, num_days, output_file=os.path.join(DATA_DIR, "financial_esg_corpus.txt")
    )
...
if st.button("🧠 Generate Insight"):
    ...
    try:
        ...
        output = gpt2.generate(
            sess, prefix=prompt, length=150, temperature=0.7, return_as_list=True
        )[0]
